...
public class RifidiEdgeServerCommands implements CommandProvider {
	...
	/**
	 * Called by spring to inject SensorManagementService
	 * 
	 * @param sensorManagementService
	 */
	public void setSensorManagementService(
			SensorManagementService sensorManagementService) {
		this.sensorManagementService = sensorManagementService;
	}
	...
	public Object _createLogicalReader(CommandInterpreter intp) {
		String logicalReaderName = intp.nextArgument();
		if (logicalReaderName == null) {
			intp
					.println("Usage: createLogicalReader <logicalReaderName> <logicalReader> [<logicalReader>]*");
			return null;
		}
		Set<String> childSensors = new HashSet<String>();
		String child = intp.nextArgument();
		if (logicalReaderName == null) {
			intp
					.println("Usage: createLogicalReader <logicalReaderName> <logicalReader> [<logicalReader>]*");
			return null;
		...
		child = intp.nextArgument();
		while (child != null) {
			childSensors.add(child);
			child = intp.nextArgument();
		}
		try {
			sensorManagementService.createSensor(logicalReaderName,
					childSensors);
			intp.println("Logical Reader: " + logicalReaderName
					+ " created sucessfully");
		} catch (DuplicateSensorNameException e) {
			intp.println("A logical sensor with name " + logicalReaderName
					+ " already exists");
